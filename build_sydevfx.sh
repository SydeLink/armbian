...
# CLEAN Test-New Kernel
./compile.sh  BOARD=rockpi-4b BRANCH=legacy RELEASE=buster BUILD_MINIMAL=no BUILD_DESKTOP=no KERNEL_ONLY=yes KERNEL_CONFIGURE=prebuilt COMPRESS_OUTPUTIMAGE=sha,gpg,img LIB_TAG=sL-stretch CLEAN_LEVEL=make,debs,oldcache,alldebs,images,cache,sources